...
CD3DX12_CPU_DESCRIPTOR_HANDLE DX12Adapter::GetRenderTargetView() const
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE DX12Adapter::GetDepthStencilView() const
{
	return CD3DX12_CPU_DESCRIPTOR_HANDLE(m_dsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart());
}

void DX12Adapter::SetNextFenceValue()
{
	// Set the fence value for the next frame.
	m_fenceValues[m_backBufferIndex] = GetCurrentFenceValue() + 1;
}

bool DX12Adapter::IsTearingAllowed()
{
...
void DX12Adapter::init(HWND window, uint32_t initFlags)
{
...
void DX12Adapter::resizeSwapChain(uint32_t width, uint32_t height, DXGI_FORMAT format)
{
...
void DX12Adapter::createSwapChain(uint32_t width, uint32_t height, DXGI_FORMAT format)
{
...
void DX12Adapter::updateColorSpace()
{
...
void DX12Adapter::initWindowSizeDependent()
{
	ASSERT(m_window);
...
void DX12Adapter::createBackBuffers()
{
...
void DX12Adapter::createDepthBuffer(uint32_t width, uint32_t height)
{
...
void DX12Adapter::enableDebugLayer() 
{
...
bool DX12Adapter::checkTearingSupport()
{
...
void DX12Adapter::createDevice(bool bEnableDebugLayer)
{
	ComPtr<IDXGIAdapter1> adapter;
...
void DX12Adapter::checkFeatureLevel()
{
...
void DX12Adapter::createCommandQueue()
{
...
void DX12Adapter::createDescriptorHeaps()
{
	// Render targets
...
void DX12Adapter::createCommandAllocators()
{
	for (uint32_t n = 0; n < m_backBufferCount; n++)
	{
...
void DX12Adapter::createCommandList()
{
...
void DX12Adapter::createEndOfFrameFence()
{
...
void present(DX12Adapter* adapter)
{
	ComPtr<ID3D12CommandAllocator> allocator = adapter->m_commandAllocators[adapter->m_backBufferIndex];
	ComPtr<ID3D12GraphicsCommandList> cmdList = adapter->m_commandList;
	ComPtr<ID3D12CommandQueue> cmdQueue = adapter->m_commandQueue;
	ComPtr<IDXGISwapChain3> swapChain = adapter->m_swapChain;
	...
	{
		char err[64] = {};
		sprintf_s(err, "Device Lost on ResizeBuffers: Reason code 0x%08X\n", (hr == DXGI_ERROR_DEVICE_REMOVED) ? adapter->m_d3dDevice->GetDeviceRemovedReason() : hr);
		ASSERT_F(false, err);
	}
	else
	{
